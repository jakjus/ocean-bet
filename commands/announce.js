...
module.exports = {
    ...
    async execute(interaction) {
        ...
        myDb.players.forEach(p => {
            let amt = p.bets.find(b => b.offerUid == offer && b.chosenOpt == result)?.amount
            if (amt) {
        ...
        toChosenString = {'team1win': chosenOffer['team1name'],
            ...
            'draw': 'Draw'}
        let winners = wins.sort((a, b) => b.ret - a.ret).map(w => `${interaction.guild.members.cache.get(w.p.userId)}: **${w.ret}💎**`).join('\n')
        myDb.offers = myDb.offers.filter(o => o.uid != chosenOffer.uid)
        db.set(interaction.guildId, myDb)
        await interaction.reply(`${interaction.user} has announced result on offer:\n${printOdds(chosenOffer)}\nResult: **${toChosenString[result]}**\n\nWinners:\n${winners}`);
    },
};
